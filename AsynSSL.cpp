...
int main(int argc, char** argv) {
	...
	if (is_client) {
		...
		int on = 1;
		if (setsockopt(ListenSocket, SOL_SOCKET, SO_REUSEADDR, &on, sizeof(on))) {
			close(ListenSocket);
	...
	int one = 1;
	if (setsockopt(ClientSocket, SOL_TCP, TCP_NODELAY, &one, sizeof(one))) {
		printf("could not setsockopt\n");
		close(ClientSocket);
		return 1;
	}
